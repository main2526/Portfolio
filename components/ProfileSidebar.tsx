...
export default function ProfileSidebar() {
  ...
  return (
    <aside className="md:-mt-[54px]">
      ...
      <p className="mt-4 text-base leading-6">{t("Developer")}</p>
      <div className="mt-4 flex items-center gap-2 text-sm"><span className="size-2 rounded-full bg-[#1f883d]" />{available("ava")}</div>
      <CvModal />
      <ul className="mt-4 space-y-1 text-sm">
        <li className="flex items-center gap-2"><BriefcaseBusiness className="size-4 text-[#656d76]" />{t("Role")}</li>
        <li className="flex items-center gap-2"><MapPin className="size-4 text-[#656d76]" />Dominican Republic</li>
        <li className="flex items-center gap-2"><Mail className="size-4 text-[#656d76]" /><a className="hover:text-[#0969da] hover:underline" href="#contact-title">{t("Contact")}</a></li>
